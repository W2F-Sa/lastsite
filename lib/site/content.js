...
export const PROJECTS = [
  {
    slug: "edge-relay",
    name: "edge-relay",
    summary: "Tiny edge-runtime helper that proxies streaming requests with proper duplex semantics.",
    stack: ["TypeScript", "Edge Runtime", "Vercel"],
    repo: "https://github.com/mahandevs/edge-relay",
    site: null,
  },
  {
    slug: "log-shaper",
...
export const POSTS = [
  {
    slug: "shipping-streaming-on-the-edge",
    title: "Shipping streaming HTTP on the edge without losing your weekend",
    date: "2025-11-18",
    tags: ["edge", "http", "streaming"],
    excerpt:
      "How duplex: \"half\" and ReadableStream finally let me build a relay that doesn't buffer the entire response in memory.",
    body: `
<p>I've been chasing the same bug for a year. Every "edge function" platform I tried claimed to support streaming, and almost none of them really did — the platform would helpfully buffer the response, or close the request body once <code>fetch</code> resolved, and my carefully written backpressure code might as well have been a comment.</p>
<p>The fix turned out to be embarrassingly small. The runtime has to support <code>fetch(url, { duplex: "half", body: req.body })</code> and the upstream has to be HTTP/2 capable. Once both are true, the edge function does almost nothing:</p>
<pre><code>return await fetch(target, {
  method: req.method,
...
<p>That's it. The trick is what you <em>don't</em> do: no <code>await req.text()</code>, no <code>new Request(...)</code> wrapper, no header normalisation that secretly clones the stream. Every allocation you avoid is a microsecond you don't have to defend at p99.</p>
<h2>What about backpressure?</h2>
<p>WebStreams give you backpressure for free if you stay out of the way. <code>req.body</code> is a <code>ReadableStream</code>, the upstream <code>fetch</code> consumes it lazily, and as long as you return the upstream <code>Response</code> directly the runtime wires the read side to the client without an intermediate buffer.</p>
<h2>Header hygiene</h2>
<p>Edge platforms love sprinkling <code>x-vercel-*</code>, <code>cf-*</code>, <code>x-real-ip</code>, and friends onto incoming requests. Strip all of them. Forward a single, normalised <code>x-forwarded-for</code> if you need the client IP and let the upstream decide what to do with it. Forwarding the platform's own forwarding headers is how you end up with a request that looks nothing like what the upstream expects.</p>
<h2>The takeaway</h2>
<p>If your "streaming proxy" is a 200-line file you don't trust, delete it and write the 40-line version. Boring code is the only kind that survives a 3am pager.</p>
`,
  },
  {
